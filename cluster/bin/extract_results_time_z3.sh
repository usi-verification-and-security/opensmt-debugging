...
for file in $1/z3.*.smt2.bz2.sh.*.out; do
    name=$(echo $file |sed 's,'$1'/z3\.\(.*\)\.smt2\.bz2\.sh\.\(.*\).out,\1,g');
    num=$(echo $file |sed 's,'$1'/z3\.\(.*\)\.smt2\.bz2\.sh\.\(.*\).out,\2,g');
    ...
    dn=$(dirname $file)
    tf=${dn}/z3.${name}.smt2.bz2.sh.osmt.${num}.time
